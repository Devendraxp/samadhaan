...
export default function CleaningLayout({ children }: { children: ReactNode }) {
  ...
  const links: DashboardLink[] = [
    { href: "/dashboard/cleaning", name: "Home", icon: <Home className="size-4" /> },
    { href: "/dashboard/cleaning/notifications", name: "Notifications", icon: <Bell className="size-4" /> },
  ];
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen">
      <DashboardSidebar user={{ name: user.name, role: "cleaning", image: user.image || staffIcon }} links={links} />
