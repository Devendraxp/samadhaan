...
export default function TransportLayout({ children }: { children: ReactNode }) {
  ...
  const links: DashboardLink[] = [
    { href: "/dashboard/transport", name: "Home", icon: <Home className="size-4" /> },
    { href: "/dashboard/transport/notifications", name: "Notifications", icon: <Bell className="size-4" /> },
];
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen">
      <DashboardSidebar user={{ name: user.name, role: "transport", image: user.image || staffIcon }} links={links} />
