...
export default function ArchivedComplaintsPage() {
  ...
  const patchStatus = async (id: string, status: ComplaintStatus) => {
    if (!serverUrl) {
      toast.error("Missing server URL");
      return;
    }
    try {
      const res = await axios.patch(`${serverUrl}/complaint/${id}`, { status }, { withCredentials: true });
  ...
  const list = grouped.ARCHIVED;
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Archived complaints</h1>
        <Button size="sm" variant="outline" onClick={fetchAll} disabled={loading}>Refresh</Button>
      </div>
      {loading ? (
        <div className="p-4 text-sm">Loading...</div>
      ) : list.length === 0 ? (
        <div className="p-4 text-sm text-muted-foreground">No archived complaints.</div>
      ) : (
              ...
              .map((c) => (
                <Card key={c.id} className="p-3 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="xs" variant="secondary" className="p-2 m-2" onClick={() => patchStatus(c.id, "CREATED")}>
                      Revert to CREATED
                    </Button>
                    <Button size="xs" variant="ghost" className="p-2 m-2" onClick={() => patchStatus(c.id, "DELETED")}>
                      Delete
                    </Button>
                  </div>
                </Card>
